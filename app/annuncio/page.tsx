...
export default function Annuncio() {
  useEffect(() => {
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .cols{display:grid;grid-template-columns:1fr 392px;gap:56px;align-items:start;}
        .chips{display:flex;flex-wrap:wrap;gap:9px;margin-bottom:44px;}
        .chip{display:inline-flex;align-items:center;gap:8px;font-size:14.5px;font-weight:600;padding:11px 18px;border-radius:100px;background:#fff;border:1px solid var(--line);}
        .chip svg{color:var(--rosso);}
        .sec{margin-bottom:54px;scroll-margin-top:104px;}
        .sec-h{display:flex;align-items:baseline;gap:14px;margin-bottom:24px;}
        ...
        .desc p{color:#474D59;font-weight:400;font-size:17px;line-height:1.78;margin-bottom:18px;max-width:660px;}
        .desc p strong{color:var(--ink);font-weight:600;}
        .facts{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:0;}
        .fact{background:#fff;border:1px solid var(--line);border-radius:16px;padding:18px 18px 15px;}
        .fact small{display:block;font-size:11.5px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--mute);margin-bottom:5px;}
        .fact b{font-family:var(--display);font-size:22px;font-weight:600;letter-spacing:-.01em;}
        .spec-group{background:#fff;border:1px solid var(--line);border-radius:18px;overflow:hidden;}
        .spec-group + .spec-group{margin-top:14px;}
        .spec-head{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:18px 24px;cursor:pointer;user-select:none;}
        .spec-head h3{font-family:var(--display);font-size:20px;font-weight:600;letter-spacing:-.01em;}
        .spec-head svg{transition:transform .35s var(--ease);color:var(--mute);flex-shrink:0;}
        .spec-group.open .spec-head svg{transform:rotate(180deg);}
        .spec-rows{display:none;border-top:1px solid var(--line);}
        .spec-group.open .spec-rows{display:block;}
        .spec-row{display:grid;grid-template-columns:1fr 1.3fr;gap:18px;padding:14px 24px;font-size:15.5px;}
        .spec-row:nth-child(even){background:var(--paper);}
        .spec-row dt{color:#565C68;font-weight:400;}
        .spec-row dd{font-weight:600;}
        .spec-row dd.ok{color:#168752;display:flex;align-items:center;gap:7px;}
        .model-widget{position:relative;background:var(--ink);color:var(--paper);border-radius:20px;padding:30px 32px;display:flex;align-items:center;justify-content:space-between;gap:24px;overflow:hidden;transition:transform .4s var(--ease);}
        ...
        .mw-title{font-family:var(--display);font-size:27px;font-weight:500;}
        .mw-title em{font-style:italic;color:var(--rosso);}
        .mw-sub{font-size:14.5px;color:rgba(250,248,244,.6);font-weight:300;margin-top:6px;max-width:380px;}
        .mw-arrow{flex-shrink:0;width:54px;height:54px;border-radius:50%;border:1.5px solid rgba(250,248,244,.3);display:flex;align-items:center;justify-content:center;transition:all .35s var(--ease);}
        ...
        .side-card{background:#fff;border:1px solid var(--line);border-radius:20px;padding:26px;}
        .seller{display:flex;align-items:center;gap:14px;margin-bottom:18px;}
        .seller-avatar{width:54px;height:54px;border-radius:50%;background:var(--ink);color:#fff;font-family:var(--display);font-weight:600;font-size:19px;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
        ...
        .side-stats{display:flex;flex-direction:column;gap:9px;font-size:14.5px;color:var(--slate);}
        ...
        .cform h3{font-family:var(--display);font-size:22px;font-weight:600;margin-bottom:4px;letter-spacing:-.01em;}
        .cform h3 em{font-style:italic;color:var(--rosso);}
        .fld{border:1.5px solid var(--line);border-radius:13px;padding:11px 15px 9px;background:#fff;transition:border-color .25s,box-shadow .25s;}
        .fld:focus-within{border-color:var(--ink);box-shadow:0 0 0 3px rgba(12,14,18,.07);}
        .fld label{display:block;font-size:11.5px;font-weight:800;letter-spacing:.07em;text-transform:uppercase;color:var(--ink);margin-bottom:2px;}
        .fld input,.fld textarea{width:100%;border:none;background:none;font-family:var(--body);font-size:15.5px;color:var(--slate);outline:none;resize:vertical;}
        .cform .consent{display:flex;gap:9px;align-items:flex-start;font-size:12.5px;color:var(--mute);line-height:1.45;}
        .cform .consent input{margin-top:3px;accent-color:var(--rosso);}
        .form-ok{display:none;align-items:center;gap:10px;background:#EAF7F0;border:1px solid #BFE6D2;color:#168752;font-size:14px;font-weight:600;border-radius:13px;padding:13px 16px;}
        ...
        @media(max-width:640px){
          ...
          .spec-row{grid-template-columns:1fr;gap:2px;padding:13px 18px;}
        ...
        <div className="wrap">
          <div className="cols">
            <div>
              ...
              <div className="sec rv" id="specifiche">
                ...
                {[
                  ...
                  {title:'Motore e meccanica', open:false, rows:[['Motore','4.943 cc, 12 cilindri boxer (Tipo F113)'],['Potenza','390 CV a 6.300 giri'],['Cambio','Manuale 5 marce'],['Matching numbers','✓ Sì — certificato'],['Cinghie distribuzione','Sostituite 2024'],['Frizione','~90%'],['Pneumatici','Michelin TRX, 2023'],['Lavori da eseguire','Nessuno']]},
                  {title:'Carrozzeria e vernice', open:false, rows:[['Colore','Rosso Corsa (originale)'],['Stato conservazione','9 / 10'],['Vernice','Originale, riverniciatura parziale cofano (documentata)'],['Sottoscocca','✓ Eccellente, mai incidentata'],['Vetri','Originali'],['Cerchi','Originali monodado']]},
                  {title:'Interni e documentazione', open:false, rows:[['Interni','Pelle nera — 9 / 10'],['Cruscotto','Perfetto, senza crepe'],['Trousse attrezzi','✓ Presente, completa'],['Libretto tagliandi','✓ Completo e timbrato'],['Manuali','Uso e manutenzione, portadocumenti']]},
                ].map((g, gi) => (
                    ...
                    <dl className="spec-rows">
                      {g.rows.map(([k,v],ri) => (
                        <div className="spec-row" key={ri}>
                          <dt>{k}</dt>
                          <dd className={v.startsWith('✓') ? 'ok' : ''}>{v.startsWith('✓') ? <><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.6"><polyline points="20 6 9 17 4 12"/></svg>{v.slice(2)}</> : v}</dd>
                        </div>
                      ))}
                    </dl>
                  </div>
              ...
              <div className="sec rv" id="certificazioni">
                ...
                <div className="facts" style={{gridTemplateColumns:'repeat(3,1fr)'}}>
                  <div className="fact"><small>Ferrari Classiche</small><b style={{color:'#168752'}}>Certificata 2022</b></div>
                  <div className="fact"><small>ASI</small><b style={{color:'#168752'}}>Targa Oro</b></div>
                  <div className="fact"><small>Storico</small><b style={{color:'#168752'}}>Verificato</b></div>
                </div>
              </div>
              ...
              <div className="sec rv">
                <a href="#" className="model-widget" style={{display:'flex'}}>
                  ...
                  <span className="mw-arrow"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></span>
                </a>
              </div>
            </div>
            ...
            <aside className="sidebar">
              ...
              <div className="side-card rv" id="contatta" style={{scrollMarginTop:'104px'}}>
                <form className="cform" id="contactForm">
                  ...
                  <button type="submit" className="btn btn-rosso" style={{padding:'15px',fontSize:'15.5px'}}>Invia richiesta</button>
